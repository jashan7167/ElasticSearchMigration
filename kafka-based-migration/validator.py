...
def validate_migration(indices: List[str] = None, sample_size: int = 5) -> str:
    """
    Main entry point for validation.
    
    Args:
        indices: List of indices to validate. If None, auto-discovers from new cluster.
        sample_size: Number of documents to sample per index.
    
    Returns:
        Full validation report text.
    """
    logger.info("Starting migration validation...")
    
    # Auto-discover indices if not provided
    if not indices:
        new = get_new_client()
        resp = new._get("/_cat/indices?format=json").json()
        indices = [idx["index"] for idx in resp if not idx["index"].startswith(".")]
        logger.info("Auto-discovered %d indices: %s", len(indices), indices)
    
    if not indices:
        logger.warning("No indices to validate!")
        return "No indices to validate"
    
    # Run validations
    count_report = compare_counts(indices)
    
    sample_results = {}
    for idx in indices:
        sample_results[idx] = sample_docs(idx, sample_size)
    
    cluster_health = check_new_cluster_health()
    
    # Generate and return report
    return print_report(count_report, sample_results, cluster_health)