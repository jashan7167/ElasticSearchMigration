...
def _strip_type_wrapper(raw_mapping: Dict[str, Any]) -> Dict[str, Any]:
    """
    6.x raw mapping from API looks like:
        { "index_name": { "mappings": { "my_type": { "properties": { … } } } } }
    or after index key is stripped:
    ...
    """
    # Handle full API response with index name wrapper
    if len(raw_mapping) == 1:
        key = next(iter(raw_mapping))
        if key not in ("mappings", "properties", "_source"):
            # This is likely the index name wrapper, unwrap it
            raw_mapping = raw_mapping[key]
    
    # Handle "mappings" wrapper from ES 6.x
    if "mappings" in raw_mapping and "properties" not in raw_mapping:
        raw_mapping = raw_mapping["mappings"]
    
    # Already unwrapped / 7.x+ style
    if "properties" in raw_mapping:
        return raw_mapping

    # Find the single type key and unwrap (e.g., "general", "doc", "_doc")
    keys = [k for k in raw_mapping if k not in ("_source", "_meta")]
    if len(keys) == 1:
